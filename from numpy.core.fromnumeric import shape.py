...
def extractCoordinates(results, landmark_number):
    x = int(results.multi_face_landmarks[0].landmark[landmark_number].x * imageSize[0])
    y = int(results.multi_face_landmarks[0].landmark[landmark_number].y * imageSize[1])
    return [x, y]

# drawing a circle using OpenCV
def drawCircle(image, location, i=1):
    cv2.circle(image, location, 3, (0, 0, 255), -1)
    # cv2.putText(image,str(i), location, cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA)
...
def drawArrow(image, start, end):
    ...
    cv2.arrowedLine(image, end, start, (0, 255, 0), 1)
...
def addText(image,start,end,s):
    drawArrow(image,start,end)
    x_coordinate = int((start[0] + end[0])/3)
    y_coordinate=int(start[1])
    if(s=="eyebrow_chin_distance"):
        x_coordinate=int(start[0])
        y_coordinate = int((start[1] + end[1])/2)
    cv2.putText(image, s,(x_coordinate,y_coordinate), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 1, cv2.LINE_AA)
...
# the eye keypoint circle 
    ...
    # jawContour Line 2
    jawContourLine2Point1 = extractCoordinates(results, keypoints_mapping["jawContourLine2"][0])
    jawContourLine2Point2 = extractCoordinates(results, keypoints_mapping["jawContourLine2"][2])
    drawArrow(clone,jawContourLine2Point1, jawContourLine2Point2)
    addText(distance_mapping,jawContourLine2Point1, jawContourLine2Point2,"jawCounterLine2")
    addText(clone,jawContourLine2Point1, jawContourLine2Point2, "This is test")
    jaw_contour_spread_width_2 = findDistance(extractCoordinates(results, keypoints_mapping["jawContourLine2"][0]), extractCoordinates(results, keypoints_mapping["jawContourLine2"][2]))
    new_distance_dict["jaw_contour_spread_width_2"] = jaw_contour_spread_width_2
    
    # jawContour Line 3
    jawContourLine3Point1 = extractCoordinates(results, keypoints_mapping["jawContourLine3"][0])
    jawContourLine3Point2 = extractCoordinates(results, keypoints_mapping["jawContourLine3"][2])
    drawArrow(clone,jawContourLine3Point1, jawContourLine3Point2)
    addText(distance_mapping,jawContourLine3Point1, jawContourLine3Point2,"jawCounterLine3")
    jaw_contour_spread_width_3 = findDistance(extractCoordinates(results, keypoints_mapping["jawContourLine3"][0]), extractCoordinates(results, keypoints_mapping["jawContourLine3"][2]))
    new_distance_dict["jaw_contour_spread_width_3"] = jaw_contour_spread_width_3

    # left and right inner eye
    drawArrowRed(clone, extractCoordinates(results, keypoints_mapping["left_eye"][0]), extractCoordinates(results, keypoints_mapping["right_eye"][8]))
    left_right_inner_eye_width = findDistance(extractCoordinates(results, keypoints_mapping["left_eye"][0]), extractCoordinates(results, keypoints_mapping["right_eye"][8]))
    ...
    # nose bottom to chin
    drawArrowRed(clone, extractCoordinates(results,keypoints_mapping["vertical_line_more"][13]), extractCoordinates(results,keypoints_mapping["vertical_line_more"][27]))
    nose_bottom_to_chin = findDistance(extractCoordinates(results, keypoints_mapping["vertical_line_more"][13]), extractCoordinates(results, keypoints_mapping["vertical_line_more"][27]))
    new_distance_dict["nose_bottom_to_chin"] = nose_bottom_to_chin
    
    # eyebrow center to nose bottom
    drawArrowRed(clone, extractCoordinates(results,keypoints_mapping["vertical_line_more"][2]), extractCoordinates(results,keypoints_mapping["vertical_line_more"][13]))
    eyebrow_center_to_nose_bottom = findDistance(extractCoordinates(results, keypoints_mapping["vertical_line_more"][2]), extractCoordinates(results, keypoints_mapping["vertical_line_more"][13]))
    new_distance_dict["eyebrow_center_to_nose_bottom"] = eyebrow_center_to_nose_bottom 

    # nose bottom to mouth bottom
    drawArrowRed(clone, extractCoordinates(results,keypoints_mapping["vertical_line_more"][13]), extractCoordinates(results,keypoints_mapping["vertical_line_more"][22]))
    nose_bottom_to_mouth_bottom = findDistance(extractCoordinates(results, keypoints_mapping["vertical_line_more"][13]), extractCoordinates(results, keypoints_mapping["vertical_line_more"][22]))
    new_distance_dict["nose_bottom_to_mouth_bottom"] = nose_bottom_to_mouth_bottom

    # mouth length
    drawArrowRed(clone, extractCoordinates(results,keypoints_mapping["vertical_line_more"][15]), extractCoordinates(results,keypoints_mapping["vertical_line_more"][22]))
    mouth_vertical_length = findDistance(extractCoordinates(results, keypoints_mapping["vertical_line_more"][15]), extractCoordinates(results, keypoints_mapping["vertical_line_more"][22]))
    ...
    # Contour lines 
    drawLinePointSlope(clone,extractCoordinates(results,keypoints_mapping["vertical_line_more"][13]),ear_ear_angle,ear_ear_distance*(0.4))
    drawLinePointSlope(clone,extractCoordinates(results,keypoints_mapping["vertical_line_more"][22]),ear_ear_angle,ear_ear_distance*(0.3))
    ...
    center_eyebrow = [(right_eyebrow[0] + left_eyebrow[0])//2, (right_eyebrow[1] + left_eyebrow[1])//2]
    ...
    addText(distance_mapping,left_virtualline,right_virtualline,"virtualline_distance")
    ...
    right_ear = extractCoordinates(results, keypoints_mapping["right_ear_to_nose"][0])
    nose = extractCoordinates(results, keypoints_mapping["right_ear_to_nose"][1])
    right_ear_nose_distance = findDistance(right_ear, nose)
    right_ear_nose_angle = findAngle(right_ear, nose)
    right_nose = extractCoordinates(results, keypoints_mapping["right_ear_to_nose"][1])
    ...
    left_ear = extractCoordinates(results, keypoints_mapping["nose_to_left_ear"][0])
    nose = extractCoordinates(results, keypoints_mapping["nose_to_left_ear"][1])
    left_ear_nose_distance = findDistance(left_ear, nose)
    left_ear_nose_angle = findAngle(left_ear, nose)
    left_nose = extractCoordinates(results, keypoints_mapping["left_ear_to_nose"][1])
    ...
    top_lip = extractCoordinates(results, keypoints_mapping["upper_lip_to_lower_lip"][0])
    bottom_lip = extractCoordinates(results, keypoints_mapping["upper_lip_to_lower_lip"][1])
    ...
    right_eye_top = extractCoordinates(results, keypoints_mapping["right_eye_cord"][0])
    right_eye_bottom = extractCoordinates(results, keypoints_mapping["right_eye_cord"][1])
    righteye_to_righteye_distance = findDistance(right_eye_top , right_eye_bottom)
    ...
    left_eye_top = extractCoordinates(results, keypoints_mapping["left_eye_cord"][0])
    left_eye_bottom = extractCoordinates(results, keypoints_mapping["left_eye_cord"][1])
    ...
    vertical_1 = extractCoordinates(results, keypoints_mapping["vertical_line"][0])
    vertical_2 = extractCoordinates(results, keypoints_mapping["vertical_line"][1])
    ...
    return distance_dict, angle_dict, clone, distance_mapping, keypoints, new_distance_dict
...
with st.sidebar:
    ...
    app_mode = st.selectbox("Please select from the following", ["Company Info", "Project Demo", "Project Details"])

# company info mode
if app_mode == "Company Info":
    st.header("CKM VIGIL Pvt. Ltd.")
    st.subheader("The company based out of India working on different aspects of Computer Vision.")
    st.write("For more details visit our website [ckmvigil.in](https://ckmvigil.in/)")
...
# project demo mode
    ...
    # images upload 
    image_1 = st.file_uploader("Choose an Image 1", type=["jpg", "png", "jpeg"], key = "image1")
    image_2 = st.file_uploader("Choose an Image 2", type=["jpg", "png", "jpeg"], key = 'image2')
    ...
    options_selected = st.sidebar.multiselect('Which result you want to get', ["Keypoints", "Characteristics", "Distance", "Angle", "Mouth Position", "Eyes Position", "Head Position", "Face Position", "New Results", "Distance Labels"])
    ...
    if image_1 is not None:
        ...
        if results.detections:
            distance_dict, angle_dict, clone, distance_mapping, keypoints, new_distance_dict = main(image_1)
    ...
    if image_2 is not None:
        ...
        if results.detections:
            distance_dict_1, angle_dict_1, clone, distance_mapping_1, keypoints, new_distance_dict_1 = main(image_2)
            ...
            if "Mouth Position" in options_selected and image_1 is not None:
                if ((distance_dict["lip_to_lip_distance"]) < (distance_dict_1["lip_to_lip_distance"])):
                    st.subheader("Mouth is open in second image with respect to image 1")  
                elif((distance_dict["lip_to_lip_distance"]) > (distance_dict_1["lip_to_lip_distance"])):
                    st.subheader("Mouth is closed in second image with respect to image 1")  
                else:
                    st.subheader("Error 404: No changes found!")    
            
            # results for eye position 
            if "Eyes Position" in options_selected and image_1 is not None:
                x=(distance_dict["righteye_to_righteye_distance"] + distance_dict["lefteye_to_lefteye_distance"])
                y=distance_dict_1["righteye_to_righteye_distance"] + distance_dict_1["lefteye_to_lefteye_distance"]
                if (x<y):
                    st.subheader("Eyes are open in second image with respect to image 1")
                else:
                    st.subheader("Eyes are closed in second image with respect to image 1")

            # results for head position
            if "Head Position" in options_selected and image_1 is not None:
                a=(distance_dict["rightear_to_nose_distance"]) / (distance_dict["leftear_to_nose_distance"])
                b=(distance_dict_1["rightear_to_nose_distance"]) / (distance_dict_1["leftear_to_nose_distance"])

                if(b>a):
                    st.subheader("Face is turned to left in second image with respect to image 1")
                elif(a>b):
                    st.subheader("Face is turned to right in second image with respect to image 1")
                else:
                    st.subheader("The head is in same position in second image with respect to image 1")

            # results for the face position 
            if "Face Position" in options_selected and image_1 is not None:
                c=(distance_dict["head_distance"]) / (distance_dict["bottomhead_distance"])
                d=(distance_dict_1["head_distance"]) / (distance_dict_1["bottomhead_distance"])

                if(c>d):
                    st.subheader("Face is upwards in second image with respect to image 1")
                elif(d>c):
                    st.subheader("Face is downwards in second image with respect to image 1")
                else:
                    st.subheader("The face is in same position in second image with respect to image 1")

                if(((angle_dict["vertical_angle"])-(angle_dict_1["vertical_angle"]))>6):
                    st.subheader("The Face has tilted to right in second image with respect to image 1")
                elif(((angle_dict["vertical_angle"])-(angle_dict_1["vertical_angle"]))<-6):
                    st.subheader("The Face has tilted to left in second image with respect to image 1")
                else:
                    st.subheader("No major change in the tilting of face in second image with respect to image 1")
...
# project details page
    ...
    st.subheader("CKM VIGIL Face API is solution that estimates 468 3D face landmarks in real-time. It only requires a simple face image.")
    st.write("Check our more projects on [ckmvigil.in/project](https://ckmvigil.in/project)")
